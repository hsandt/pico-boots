...
describe('circular_buffer', function ()
  ...
  describe('stateless_iter', function ()

    it('(^):stateless_iter() => nil', function ()
      local cb = circular_buffer(2)
      assert.is_nil(cb:stateless_iter(0))
    end)

    it('(^1):stateless_iter() => 1, nil', function ()
      local cb = circular_buffer(2)
      cb:push(1)
      assert.are_same({1, 1}, {cb:stateless_iter(0)})
      assert.is_nil(cb:stateless_iter(1))
    end)

    it('(4, ^2, 3):stateless_iter() => 2, 3, 4, nil', function ()
      local cb = circular_buffer(3)
      cb:push(1)
      cb:push(2)
      cb:push(3)
      cb:push(4)
      assert.are_same({1, 2}, {cb:stateless_iter(0)})
      assert.are_same({2, 3}, {cb:stateless_iter(1)})
      assert.are_same({3, 4}, {cb:stateless_iter(2)})
      assert.is_nil(cb:stateless_iter(3))
    end)

  end)

  describe('rotate_indice', function ()

    it('rotate_indice(-2, 3) => 1', function ()
      assert.are_equal(1, circular_buffer.rotate_indice(-2, 3))
    end)

    it('rotate_indice(-1, 3) => 2', function ()
      assert.are_equal(2, circular_buffer.rotate_indice(-1, 3))
    end)

    it('rotate_indice(0, 3) => 3', function ()
      assert.are_equal(3, circular_buffer.rotate_indice(0, 3))
    end)

    it('rotate_indice(1, 3) => 1', function ()
      assert.are_equal(1, circular_buffer.rotate_indice(1, 3))
    end)

    it('rotate_indice(2, 3) => 2', function ()
      assert.are_equal(2, circular_buffer.rotate_indice(2, 3))
    end)

    it('rotate_indice(3, 3) => 3', function ()
      assert.are_equal(3, circular_buffer.rotate_indice(3, 3))
    end)

    it('rotate_indice(4, 3) => 1', function ()
      assert.are_equal(1, circular_buffer.rotate_indice(4, 3))
    end)
