...
describe('vlogger', function ()
  ...
  describe('window (with buffer size 3)', function ()
    ...
    describe('push_msg', function ()
      ...
      it('should do nothing when message queue has not been initialized', function ()
        local lm = log_msg(logging.level.info, "flow", "enter stage state")
        window:push_msg(lm)
        assert.spy(circular_buffer.push).was_not_called()
        assert.spy(window._on_msg_pushed).was_not_called()
        assert.spy(window._on_msg_popped).was_not_called()
      end)

      describe('(when vlogger has initialized message queue)', function ()
        ...
        describe('(when queue is empty)', function ()
          ...
          it('should push a message to queue and vertical layout', function ()
            local lm = log_msg(logging.level.info, "flow", "enter stage state")
            window:push_msg(lm)
            assert.spy(circular_buffer.push).was_called(1)
            assert.spy(circular_buffer.push).was_called_with(match.ref(window._msg_queue), lm)
