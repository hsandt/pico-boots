...
describe('animated_sprite_object', function ()
  ...
  describe('init', function ()
    ...
    it('should init a sprite with animated sprite data, visible, default position: (0, 0), default scale: 1', function ()
      ...
      assert.are_same({true, vector(0, 0), 1}, {spr_object.visible, spr_object.position, spr_object.scale})
    end)

    it('should init a sprite with animated sprite data, visible, position: (2, 3) (copy), scale: 2', function ()
      local dummy_animated_spr_data = {}
      local position = vector(2, 3)
      local spr_object = animated_sprite_object(dummy_animated_spr_data, position, 2)
      -- normally we should spy on base constructor, but to simplify just check that data table was set
      assert.are_equal(dummy_animated_spr_data, spr_object.data_table)
      assert.are_same({true, vector(2, 3), 2}, {spr_object.visible, spr_object.position, spr_object.scale})
  ...
  describe('draw', function ()
    ...
    it('should delegate drawing to base animated_sprite.render', function ()
      ...
      assert.spy(animated_sprite.render).was_called_with(match.ref(spr_object), vector(2, 3), false, false, 0, 2)
    end)
